...
tripstxt = open('%s/trips.txt'%(operator), 'a')
...
#            if thisdate == today:
        ...
        for (internal, datum) in dates.items():
            ...
            for direction in directions.keys():
                ...
                while True:
                    cached = incache2(operator, modality, str(directions[direction]), datum, offset)
                    if not cached:
                        cached = incache(operator, modality, str(directions[direction]), datum, offset)
                        if not cached:
                            ...
                            if response['status'] == '200':
                                headers['Cookie'] = response['set-cookie']
                                tmp(content, operator, modality, str(directions[direction]), datum, offset)
                                eventvalidation, viewstate = fetchevent(content)
                                cached = content

                        prevoffset = offset
                        lasttripid, offset, last, result = parsetimetable(cached, lasttripid, offset, last)
                        tmp2([lasttripid, offset, last, result], operator, modality, str(directions[direction]), datum, prevoffset)
                    else:
                        prevoffset = offset
                    ...
                    if len(result) > 0:
                        ...
                        for (tripid,v) in result.items():
                            ...
                            for trip in v:
                                needle = 'F_'+str(directions[direction])+'_'+trip[2]
            ...
            for tripid in tripids:
                trips('100', calendarmagic(calendar, start_date, end_date), str(tripid), '', '', str(directions[direction]), '', '')
